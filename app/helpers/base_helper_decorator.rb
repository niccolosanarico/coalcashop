    ...
    def link_to_cart(text = nil)
      ...
      text = text ? h(text) : Spree.t('cart')
        ...
        if simple_current_order.nil? or simple_current_order.item_count.zero?
          ...
          text = "<i class='fa fa-shopping-cart fa-2x' aria-hidden='true'></i> #{text}: (#{Spree.t('empty')})"
          css_class = 'empty'
        else
          ...
          text = "<i class='fa fa-shopping-cart fa-2x' aria-hidden='true'></i> #{text}: (#{simple_current_order.item_count})  <span class='amount'>#{simple_current_order.display_total.to_html}</span>"
          css_class = 'full'
        end
